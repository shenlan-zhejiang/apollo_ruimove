...
class DataCollector(object):
    ...
    def safty_check(self,cmd):
        self.cmd = list(map(float, cmd))
        if self.cmd[0] > 20:
            print('The throttle is recommended not to exceed 20.') 
            return False
        elif self.cmd[1] > 2:
            print('The velocity is recommended not to exceed 2.')
            return False
        elif self.cmd[2] > 0:
            print('The brake number should be negative.')
            return False
        else:
            return True
...
def main():
    ...
    while True:
        ...
        if len(cmd) == 0:
            ...
            bsafty = data_collector.safty_check(cmd)
            if(bsafty == True):
