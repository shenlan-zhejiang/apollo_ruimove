...
ErrorCode DevkitController::Init(
  ...
  if (driving_mode() == Chassis::EMERGENCY_MODE) {
    ...
    AINFO << "Initialize from Emergency mode to Complete manual.";
  }
}
...
void DevkitController::Emergency() {
  ...
  AINFO << "Switch to Emergency mode.";
  ResetProtocol();
}
...
void DevkitController::Throttle(double pedal) {
  ...
  // if (!FLAGS_enabel_speed_mode) {

  // }
  // throttle_command_100_->set_throttle_pedal_target(pedal);
  throttle_command_100_->set_throttle_pedal_target(pedal);
  vehicle_mode_command_105_->set_drive_mode_ctrl(Vehicle_mode_command_105::DRIVE_MODE_CTRL_THROTTLE_PADDLE_DRIVE);
}
...
void DevkitController::Acceleration(double acc) {
  if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE ||
      driving_mode() != Chassis::AUTO_SPEED_ONLY) {
    // throttle_command_100_->set_throttle_pedal_target(acc);
    AINFO_EVERY(1000) << "The current drive mode does not need to set acceleration.";
    return;
  }
  // None
}
  ...
  while (can_sender_->IsRunning()) {
    ...
    if ((mode == Chassis::COMPLETE_AUTO_DRIVE ||
      ...
      if (horizontal_ctrl_fail >= kMaxFailAttempt) {
        AINFO << "H control failure exceeded max attempt.";
        emergency_mode = true;
    ...
    if ((mode == Chassis::COMPLETE_AUTO_DRIVE ||
      ...
      if (vertical_ctrl_fail >= kMaxFailAttempt) {
        AINFO << "V control failure exceeded max attempt.";
        emergency_mode = true;
        set_chassis_error_code(Chassis::MANUAL_INTERVENTION);
      }
    } else {
